...
func TestBuildReleasePlan_PersistsTagDescription(t *testing.T) {
	m, gitMock := newReleasePlanTestManager(t, &mockGitClient{})
	seedReleasePlanTasks(t, m.cfg.TasksRoot, gitMock,
	...
	plan, err := m.buildReleasePlan(t.Context(), CreateReleaseParams{
		...
		ServiceTagDescriptions: map[string]string{"api": "Fix retry after timeout"},
	})
	if err != nil {
		t.Fatalf("buildReleasePlan() error = %v", err)
	}
	if plan.Services[0].TagDescription != "Fix retry after timeout" {
		t.Fatalf("TagDescription = %q", plan.Services[0].TagDescription)
	}
}
